...
def greet_user(update, context):
    ...
    update.message.reply_text(f'Привет, пользователь {context.user_data["emoji"]}! Ты вызвал команду /start', reply_markup=my_keyboard())

def talk_to_me(update, context):
    text = update.message.text 
    print(text)
    context.user_data["emoji"] = get_smile(context.user_data)
    update.message.reply_text(f"{text} {context.user_data['emoji']}", reply_markup=my_keyboard())
